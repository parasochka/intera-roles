...
| `template-parts/partials/*.php` | Repeated page markup — one source per block. `method-band.php` is the pair the product page, the Roles hub and the three role landings all close on; it takes a `source_id` and reads its words against that page, so the Method is written in one place and quoted everywhere else. `intera_page_id()` in `inc/template-tags.php` is what resolves that page — the same lookup `intera_page_url()` uses, stopping one step earlier. |
| `page-*.php` | Named page templates (`Template Name:` header), including `page-roles.php` (the `/roles` hub) and the three `/roles/*` campaign landings under it — `page-role-sysadmin.php`, `page-role-finance.php`, `page-role-account-management.php`. They live at the theme **root**, not in `page-templates/`: `intera_page_url()` resolves a destination by looking for the template `page-<key>.php`, and a template in a subdirectory is stored as `page-templates/…` and never matches. `page-templates/` is empty and stays that way. |
